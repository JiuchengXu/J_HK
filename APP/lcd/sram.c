...
{	
    ...
    readWriteTiming.FSMC_DataSetupTime = 0x02;		 //数据保持时间（DATAST）为3个HCLK 4/72M=55ns(对EM的SRAM芯片)	 
...
#define EXTERN_SRAM_BASE 0x68000000
void sram_test(void)
{
	u32 ts;
	
	//*(volatile u8 *)EXTERN_SRAM_BASE = 0x11;
	
	//*(volatile u8 *)(EXTERN_SRAM_BASE + 1) = 0x22;
	
	//x = *(volatile u16 *)EXTERN_SRAM_BASE;
	
	//return;
	
	 for (ts = 0; ts < 0x1 * 1024 * 1024; ts++) {
		int a = EXTERN_SRAM_BASE;
		u8 c;
		c = (u8)ts & 0xff;
		*(volatile u8 *)(a + ts)= c;
	}
